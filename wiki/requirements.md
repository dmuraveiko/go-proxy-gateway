...
- Один логический Proxy имеет `proxy_id` и одну PostgreSQL. Штатно запускается один
  процесс; несколько физических экземпляров с общей БД, NATS queue group и DB leases
  остаются опциональным HA-режимом. Разные `proxy_id` используют разные БД.
...
- Один клиент может использовать одну PostgreSQL для нескольких Proxy. Клиентские
  операции разделяются по `(proxy_id, request_id)`, callback — по
  `(proxy_id, delivery_id)`. Восстановление фильтруется по `proxy_id`.
- Один Proxy обслуживает несколько клиентов; его записи и NATS subjects различаются
  по `client_id`.
...
- После handler клиент сначала отправляет callback-ответ в NATS, затем сохраняет его в
  своей БД. Сохранённый ответ восстанавливается без повторного вызова handler. Proxy
  дедуплицирует его по `event_id`/`delivery_id` и отклоняет конфликтующее содержимое.
...
- Внешний callback endpoint принимает любой HTTP method. На стороне Proxy команды
  register/update/subscribe/unsubscribe/delete и их результаты сохраняются и
  дедуплицируются. Клиент повторяет команду до ACK, пока активен вызов; автоматическое
  восстановление control-команды после рестарта клиента пока не реализовано.
