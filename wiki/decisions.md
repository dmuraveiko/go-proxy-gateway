...
## ADR-020 — Составные ключи клиентского хранилища

**Статус:** принято после review.

Один клиент может работать с несколькими Proxy через отдельный Transport на каждый
`proxy_id`, используя общую PostgreSQL. Исходящие операции идентифицируются парой
`(proxy_id, request_id)`, callback-доставки — `(proxy_id, delivery_id)`. Восстановление
незавершённых записей обязательно фильтруется по `proxy_id`.
