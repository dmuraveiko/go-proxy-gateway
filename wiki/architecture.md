...
Один логический Proxy определяется через `proxy_id` и одну PostgreSQL. Несколько его
физических экземпляров используют одну NATS queue group и общую БД. `FOR UPDATE SKIP
LOCKED`, leases и unique constraints не дают им выполнить одну операцию одновременно.
У каждого процесса есть `instance_id` для адресных callback responses.
