...
Это предлагаемая целевая архитектура после замечаний от 16 июля 2026 года. Текущий
прототип ещё требуется упростить и привести к ней.
...
идемпотентно и используют полный прикладной ACK.
...
Один Proxy-сервер имеет уникальный `proxy_id` и работает только со своей PostgreSQL.
Другой Proxy использует другой `proxy_id` и другую БД. Они могут работать в одном NATS,
потому что subjects содержат `proxy_id` и `client_id`; доступ дополнительно ограничен
NATS ACL и Ed25519 ACL.

Один долгоживущий `net/http.Transport` повторно использует соединения. RPS,
concurrency и при необходимости `min_interval` по target host хранятся в БД конкретного
Proxy.
