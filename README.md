...
- клиентская библиотека содержит готовое PostgreSQL-хранилище с таблицами
  `natsproxyclient_*`;
- записи клиентской БД разделяются составными ключами `proxy_id + request_id` и
...
Одна клиентская БД может обслуживать несколько Proxy. Исходящие операции хранятся по
ключу `(proxy_id, request_id)`, callback — по `(proxy_id, delivery_id)`. Поэтому
одинаковый ID у `proxy-a` и `proxy-b` означает две независимые записи.
...
несколько Proxy, создавая отдельный Transport с нужным `proxy_id`. Один конкретный
запрос всегда принадлежит одному Proxy, а восстановление из клиентской БД выбирает
только записи этого `proxy_id`.
